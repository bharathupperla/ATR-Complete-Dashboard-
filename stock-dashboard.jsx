...
// Accepts `stocks` prop from App (live API data) or falls back to mock STOCKS.
  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      ...
      <div style={{
        ...
        <span style={{ fontFamily:'Chakra Petch', fontWeight:700, fontSize:11, letterSpacing:'0.18em', color, textTransform:'uppercase' }}>{label}</span>
        <span style={{ fontFamily:'Space Mono', fontSize:13, color:PALETTE.text, fontWeight:700, paddingLeft:8, borderLeft:`1px solid ${color}33` }}>{count}</span>
      </div>
      {hover && tickers && tickers.length > 0 ? (
        <div style={{
          position:'absolute', top:'calc(100% + 8px)', left:'50%', transform:'translateX(-50%)',
          background:'rgba(4,8,16,0.96)', border:`1px solid ${color}55`,
...
function AtrGauge({ atr }) {
  const max = Math.max(atr.current, atr.p75) * 1.35 || 10;
  ...
  return (
    <div style={{ width:'100%' }}>
      <div style={{
        ...
        background:'#040810', borderRadius:6,
        border:`1px solid ${PALETTE.border}`,
        boxShadow:'inset 0 2px 6px rgba(0,0,0,0.7), inset 0 -1px 0 rgba(255,255,255,0.04)',
        overflow:'hidden',
      }}>
        <div style={{ position:'absolute', left:0, top:0, bottom:0, width:pct(atr.p25), background:`linear-gradient(90deg, ${PALETTE.bearish}33, ${PALETTE.bearish}10)` }} />
        <div style={{ position:'absolute', right:0, top:0, bottom:0, left:pct(atr.p75), background:`linear-gradient(270deg, ${PALETTE.bearish}33, ${PALETTE.bearish}10)` }} />
        <div style={{ position:'absolute', top:0, bottom:0, left:pct(atr.p25), width:`calc(${pct(atr.p75)} - ${pct(atr.p25)})`, background:`linear-gradient(90deg, ${PALETTE.bullish}40, ${PALETTE.bullish}25)`, boxShadow:`inset 0 0 12px ${PALETTE.bullish}33` }} />
        {[atr.p25, atr.p75].map((v, i) => (
          <div key={i} style={{ position:'absolute', top:-1, bottom:-1, left:pct(v), width:1, background:PALETTE.text, opacity:0.55 }} />
        ))}
        <div style={{
          position:'absolute', top:-3, bottom:-3, left:pct(atr.current),
    ...
    <div style={{ width:'100%', height:44 }}>
      <RC.ResponsiveContainer>
        <RC.LineChart data={series} margin={{ top:4, right:4, left:4, bottom:0 }}>
          <RC.Line dataKey="v" type="monotone" stroke={PALETTE.amber} strokeWidth={1.6}
            dot={false} isAnimationActive animationDuration={1100} animationEasing="ease-out" />
        </RC.LineChart>
      </RC.ResponsiveContainer>
    </div>
  );
...
function StockCard({ s, idx, onOpen }) {
  ...
  return (
    <div
      ...
      <div style={{ position:'absolute', inset:0, pointerEvents:'none', borderRadius:12, background:`radial-gradient(360px circle at ${tilt.mx}% ${tilt.my}%, ${meta.color}1F, transparent 60%)`, opacity:tilt.hover ? 1 : 0, transition:'opacity 200ms ease' }} />
      <div style={{ position:'absolute', left:0, top:10, bottom:10, width:3, background:meta.color, boxShadow:`0 0 14px ${meta.color}, 0 0 24px ${meta.color}66`, borderRadius:3 }} />

      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10 }}>
        <div>
          <div style={{ fontFamily:'Chakra Petch', fontWeight:700, fontSize:'clamp(22px, 2.2vw, 26px)', letterSpacing:'0.06em', color:PALETTE.text, lineHeight:1 }}>{s.ticker}</div>
          <div style={{ marginTop:4, fontFamily:'Space Mono', fontSize:10, color:PALETTE.textMute, letterSpacing:'0.14em', textTransform:'uppercase' }}>
            {s.entryDate ? fmtDate(s.entryDate) : '—'} · {s.weeksHeld}W HELD
          </div>
        </div>
      ...
      <div style={{ display:'flex', alignItems:'baseline', gap:10, marginTop:18 }}>
        <CountNumber
          value={s.returnPct} decimals={2} suffix="%" prefix={pos ? '+' : ''}
          color={retColor}
          style={{ fontWeight:700, fontSize:'clamp(34px, 4.4vw, 46px)', letterSpacing:'-0.02em', textShadow:`0 0 22px ${retColor}55` }} />
        <span style={{ fontFamily:'Space Mono', fontSize:11, color:PALETTE.textMute, letterSpacing:'0.18em', textTransform:'uppercase' }}>RET</span>
      </div>

      <div style={{ display:'flex', gap:18, marginTop:6, fontFamily:'Space Mono', fontSize:11 }}>
        <div>
          <span style={{ color:PALETTE.textMute, letterSpacing:'0.14em' }}>1W </span>
          <span style={{ color:s.weekDelta >= 0 ? PALETTE.bullish : PALETTE.bearish }}>{fmtPct(s.weekDelta)}</span>
        </div>
        <div>
          <span style={{ color:PALETTE.textMute, letterSpacing:'0.14em' }}>PRV </span>
          <span style={{ color:s.prevWeekDelta >= 0 ? PALETTE.bullish : PALETTE.bearish }}>{fmtPct(s.prevWeekDelta)}</span>
        </div>
      </div>

      <div style={{ marginTop:18 }}>
        <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'Chakra Petch', fontSize:10, color:PALETTE.textDim, letterSpacing:'0.22em', textTransform:'uppercase', marginBottom:6 }}>
          <span>ATR Gauge</span>
      ...
      <div style={{ marginTop:14 }}>
        <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'Chakra Petch', fontSize:10, color:PALETTE.textDim, letterSpacing:'0.22em', textTransform:'uppercase', marginBottom:2 }}>
          <span>13W ATR</span>
...
function SkeletonCard({ idx }) {
  return (
    <div style={{ background:PALETTE.surface, border:`1px solid ${PALETTE.border}`, borderRadius:12, padding:18, boxShadow:panelShadow(false,1), animation:`sw-card-in 600ms cubic-bezier(.2,.8,.2,1) ${idx * 50}ms both` }}>
...
function StockDrawer({ stock, onClose }) {
  ...
  const meta = SIGNAL_META[stock.signal] || SIGNAL_META.NEUTRAL;

  const retSeries = (stock.returnHistory || []).map((v, i) => ({
    i, week: `W${i + 1}`,
    ret: v,
    fit: stock.bestFitLine && stock.bestFitLine[i] != null
      ? stock.bestFitLine[i]
      : +((stock.trend.slope * i) + (stock.returnHistory[0] || 0)).toFixed(2),
  }));
  ...
  return (
    <>
      <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:90, background:'rgba(2,4,8,0.66)', backdropFilter:'blur(4px)', WebkitBackdropFilter:'blur(4px)', opacity:mounted ? 1 : 0, transition:'opacity 220ms ease' }} />
      <aside role="dialog" aria-label={`${stock.ticker} detail`} style={{
        position:'fixed',
        ...(isMobile
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 22px', borderBottom:`1px solid ${PALETTE.border}`, background:'linear-gradient(180deg, rgba(255,255,255,0.04), transparent)' }}>
          <div style={{ display:'flex', alignItems:'baseline', gap:14 }}>
            <span style={{ fontFamily:'Chakra Petch', fontWeight:700, fontSize:28, letterSpacing:'0.06em', color:PALETTE.text }}>{stock.ticker}</span>
            {stock.name && stock.name !== stock.ticker ? (
              <span style={{ fontFamily:'Space Mono', fontSize:11, color:PALETTE.textMute, letterSpacing:'0.18em', textTransform:'uppercase' }}>{stock.name}</span>
            ) : null}
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            <SignalBadge signal={stock.signal} />
            <button onClick={onClose} aria-label="Close" style={{ width:34, height:34, borderRadius:6, background:'transparent', color:PALETTE.textDim, border:`1px solid ${PALETTE.border}`, cursor:'pointer', display:'grid', placeItems:'center' }}>
        ...
        <div style={{ overflow:'auto', padding:22, flex:1 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(120px, 1fr))', gap:1, background:PALETTE.border, border:`1px solid ${PALETTE.border}`, borderRadius:8, overflow:'hidden' }}>
            ...
            <Metric label="Held"    value={`${stock.weeksHeld}W`} />
          </div>

          <SectionLabel style={{ marginTop:22 }}>Return History</SectionLabel>
          <ChartFrame height={200}>
            <span style={{ position:'absolute', top:10, right:14, zIndex:2 }}>
              <span style={{ display:'inline-flex', gap:8, background:'rgba(4,8,16,0.7)', border:`1px solid ${PALETTE.amber}55`, padding:'4px 10px', borderRadius:999, fontFamily:'Space Mono', fontSize:10, color:PALETTE.amber, letterSpacing:'0.16em' }}>
                {stock.trend.angle.toFixed(1)}° · R² {stock.trend.r2.toFixed(2)}
              </span>
            </span>
            <RC.ResponsiveContainer>
              ...
              <RC.LineChart data={atrSeries} margin={{ top:12, right:50, left:-10, bottom:0 }}>
                ...
                <RC.Line dataKey="atr" type="monotone" stroke={PALETTE.amber} strokeWidth={2} dot={false} animationDuration={1200}
                  label={({ x, y, value, index }) => index === atrSeries.length - 1 ? (
                    <text x={x + 6} y={y + 4} fill={PALETTE.amber} fontSize="10" fontFamily="Space Mono">{(value || 0).toFixed(2)}</text>
                  ) : null} />
              </RC.LineChart>
            </RC.ResponsiveContainer>
          </ChartFrame>
          ...
          <div style={{ background:'#040810', border:`1px solid ${PALETTE.border}`, borderLeft:`3px solid ${meta.color}`, borderRadius:8, padding:16, fontSize:13, lineHeight:1.6, color:PALETTE.textDim }}>
            {_explainSignal(stock)}
          </div>
        </div>
      </aside>
    </>
...
function _explainSignal(s) {
  const inZone = s.atr.current >= s.atr.p25 && s.atr.current <= s.atr.p75;
  const zone   = inZone ? 'inside' : s.atr.current < s.atr.p25 ? 'below' : 'above';
  if (s.signal === 'BUY MORE') {
    return `Trend strength is solid (R² ${s.trend.r2.toFixed(2)}, slope ${s.trend.angle.toFixed(1)}°) and ATR ${s.atr.current.toFixed(2)} sits ${zone} the healthy band [${s.atr.p25.toFixed(2)} – ${s.atr.p75.toFixed(2)}]. Return fell while volatility rose — momentum stress buy signal.`;
  }
  if (s.signal === 'SQUARE OFF') {
    return `Week deltas ${fmtPct(s.weekDelta)} / ${fmtPct(s.prevWeekDelta)} indicate momentum exhaustion. ATR ${s.atr.current.toFixed(2)} is ${s.atr.current > s.atr.p75 ? 'above the P75 ceiling — volatility expanding' : 'compressing below P25 — trend losing energy'}. Consider taking profit.`;
  }
  return `Slope angle ${s.trend.angle.toFixed(1)}° (R² ${s.trend.r2.toFixed(2)}) — low conviction trend. ATR ${s.atr.current.toFixed(2)} is ${zone} the healthy band. Hold and re-evaluate next week.`;
}

function SectionLabel({ children, style }) {
  return (
    <div style={{ fontFamily:'Chakra Petch', fontWeight:600, fontSize:11, letterSpacing:'0.26em', textTransform:'uppercase', color:PALETTE.textDim, marginBottom:10, ...style }}>{children}</div>
  );
}

function Metric({ label, value, color = PALETTE.text }) {
  return (
    <div style={{ background:PALETTE.surface, padding:'12px 14px' }}>
      <div style={{ fontFamily:'Chakra Petch', fontSize:9.5, letterSpacing:'0.22em', textTransform:'uppercase', color:PALETTE.textMute }}>{label}</div>
      <div style={{ marginTop:4, fontFamily:'Space Mono', fontWeight:700, fontSize:16, color }}>{value}</div>
    </div>
  );
}
...
function ChartFrame({ children, height = 200 }) {
  return (
    <div style={{ position:'relative', background:'#040810', border:`1px solid ${PALETTE.border}`, borderRadius:8, padding:'6px 4px', boxShadow:'inset 0 1px 0 rgba(255,255,255,0.04), inset 0 0 30px rgba(0,0,0,0.5)', height }}>{children}</div>
  );
}
...
function TermTooltip({ active, payload, label, suffix = '' }) {
  ...
  return (
    <div style={{ background:'rgba(4,8,16,0.95)', border:`1px solid ${PALETTE.border}`, borderRadius:6, padding:'8px 12px', fontFamily:'Space Mono', fontSize:11, color:PALETTE.text, boxShadow:'0 12px 28px rgba(0,0,0,0.6)' }}>
...
// Accepts `stocks` prop (live data from App) or falls back to mock STOCKS.
// =============================================================
function StockDashboard({ onBack, stocks: stocksProp, onRefresh }) {
  const stocks = stocksProp || STOCKS;
  ...
  const counts = _sUseMemo(() => {
    ...
    stocks.forEach((s) => {
      ...
      if (buckets[sig]) buckets[sig].push(s.ticker);
    });
    return buckets;
  }, [stocks]);
    ...
    <div style={{ position:'relative', minHeight:'100vh', background:PALETTE.bg, color:PALETTE.text }}>
      ...
      <div style={{ position:'relative', zIndex:5 }}>
        ...
        <main style={{ padding:'clamp(18px, 3vw, 28px)' }}>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px, 1fr))', gap:'clamp(16px, 2vw, 22px)' }}>
            {!loaded
              ? Array.from({ length: Math.max(8, stocks.length) }).map((_, i) => <SkeletonCard key={i} idx={i} />)
              : stocks.map((s, i) => <StockCard key={s.ticker + i} s={s} idx={i} onOpen={setActive} />)
            }
          </div>
        </main>
      </div>
